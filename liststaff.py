...
def display_data():
    ...
    staff_name = entry_fullname.get()
    staff_address = entry_address.get()
    staff_birth = entry_birth.get()
    staff_age = entry_age.get()
    staff_race = entry_race.get()
    staff_religion = entry_religion.get()
    staff_email = entry_email.get()
    staff_phone = entry_phone.get()
    staff_date = entry_date.get()
    staff_user = entry_username.get()
    staff_password = entry_password.get()
    
    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("SELECT id, staff_name, staff_address, staff_birth, staff_age, staff_race, staff_religion, staff_email, staff_phone,staff_date, staff_user, staff_pass FROM staff")
    rows = cursor.fetchall()
    conn.close()

    liststaff_display.delete(0, tk.END)
    for row in rows:
        id_staff, staff_name, staff_address, staff_birth, staff_age, staff_race, staff_religion,staff_email, staff_phone, staff_date, staff_user, staff_password = row
        liststaff_display.insert(tk.END, f"ID: {id_staff}")
        liststaff_display.insert(tk.END, f"Name: {staff_name}")
        liststaff_display.insert(tk.END, f"Address: {staff_address}")
        liststaff_display.insert(tk.END, f"Birth: {staff_birth}")
        liststaff_display.insert(tk.END, f"Age: {staff_age}")
        liststaff_display.insert(tk.END, f"Race: {staff_race}")
        liststaff_display.insert(tk.END, f"Religion: {staff_religion}")
        liststaff_display.insert(tk.END, f"Email: {staff_email}")
        liststaff_display.insert(tk.END, f"Phone: {staff_phone}")
        liststaff_display.insert(tk.END, f"Date Registration: {staff_date}")
        liststaff_display.insert(tk.END, f"Username: {staff_user}")
        liststaff_display.insert(tk.END, f"Password: {staff_password}")
        liststaff_display.insert(tk.END, "-" * 40)
        

def update_data():
    selected_item = liststaff_display.curselection()
    ...
    selected_item_text = liststaff_display.get(selected_item[0])
    selected_item_id = selected_item_text.split(':')[1].strip()

    conn = sqlite3.connect("database.db")
    cursor = conn.cursor()
    cursor.execute("UPDATE staff SET staff_name=?, staff_address=?, staff_birth=?, staff_age=?, staff_race=?, staff_religion=?, staff_email=?, staff_phone=?, staff_date=?, staff_user=?, staff_pass=? WHERE id=?", (new_fullname, new_address, new_birthdate, new_age, new_race, new_religion, new_email, new_phone, new_date, new_username, new_password, selected_item_id))
    conn.commit()
    conn.close()
...
def delete_data():
    ...
    selected_id = liststaff_display.get(selected_item).split(":")[1].strip()
...
window = tk.Tk()
window.title("Welcome to iLIBS: Display Staffs")
...
lbl = tkinter.Label(frame, text="List Of Staffs", font=("arial", 30, "bold"))
lbl.grid(row=0,column=0)

first_frame = tkinter.LabelFrame(frame, text="List Staffs")
...
liststaff_display = tkinter.Listbox(first_frame, width=65, height=27)
liststaff_display.grid(row=0,column=0, padx=10, pady=10, sticky="news")

toolsframe = tkinter.LabelFrame(frame, text="Tools")
toolsframe.grid(row=2,column=0,padx=10)

display_button = tkinter.Button(toolsframe, text="Display Staffs", command=display_data, bg="#800080", fg="white")
display_button.grid(row=0,column=0)

update_button = tkinter.Button(toolsframe, text="Update Staff", command=update_data, bg="#1E90FF", fg="black")
