...
def create_table():
    ...
    cursor = conn.cursor()
    cursor.execute("CREATE TABLE IF NOT EXISTS staff (id INTEGER PRIMARY KEY AUTOINCREMENT, staff_name TEXT, staff_address TEXT, staff_birth TEXT, staff_age INTEGER, staff_race TEXT, staff_religion TEXT, staff_email TEXT, staff_phone TEXT, staff_user TEXT, staff_date TEXT, staff_pass TEXT)")
    conn.commit()
    conn.close()
...
def insert_data():
    ...
    if accepted=="Accepted":
        ...
        staff_pass = entry_password.get()
        
        if staff_name and staff_address and staff_birth and staff_age and staff_race and staff_religion and staff_email and staff_phone and staff_date and staff_user and staff_pass:
        
            conn = sqlite3.connect("database.db") 
            cursor = conn.cursor()
            cursor.execute("INSERT INTO staff (staff_name,staff_address,staff_birth,staff_age,staff_race,staff_religion,staff_email,staff_phone,staff_date,staff_user,staff_pass) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(staff_name,staff_address,staff_birth,staff_age,staff_race,staff_religion,staff_email,staff_phone,staff_date,staff_user,staff_pass))    
            conn.commit()
            conn.close()
...
# Main Window        
window = tk.Tk()
window.title("Welcome to iLIBS: Sign Up Staff")
...
home_buttom = tkinter.Button(frame, text="BACK",command=back)
home_buttom.grid(row=0,column=1,sticky="E")

lbl1 = tkinter.Label(frame, text="STAFF REGISTRATION", font=("Arial", 30, "bold"), bg="#FF6A6A")
...
# Account Frame
account_frame = tkinter.LabelFrame(frame, text="Create New Account as Staff")
